...
// Schema keys
const WEATHER_GET_LOCATION = "getLocation"
...
, loadJsonAsync: function loadJsonAsync(url, callback) {
  ...
  displayWeather: function() {
    let mainCondition = weather.condition.main;
    let descriptionCondition = this.capitalizeFirstLetter(weather.condition.description);
    // Translating
    ...
    // Displaying Location   
    let location = weather.location.city + ", " + weather.location.country;
    ...
    let temp = this.TempToUserUnits(weather.main.temperature);
    // Applet Label
    let label = "";
...
, rebuildCurrentWeatherUi: function rebuildCurrentWeatherUi() {
    ...
    this._currentWeatherApiUnique = new St.Label(textOb)

    // APi Unique Caption
    this._currentWeatherApiUniqueCap = new St.Label({text: _('')});
...
, rebuildFutureWeatherUi: function rebuildFutureWeatherUi() {
  ...
  TempToUserUnits: function(kelvin) {
    ...
    if (this._temperatureUnit == WeatherUnits.FAHRENHEIT) {
      return Math.round((9/5*(kelvin - 273) + 32) * 10)/ 10;
    }
  },
...
//////////////////////////////////////////////////////////////
  ...
  getOpenWeatherCurrentWeather: function() {  
    ...
    this.loadJsonAsync(query, function(json) {
      ...
      if (json.cod != 200) {
        ...
        if (json.cod == 404) {
          this._currentWeatherSummary.text = "City Not found";
        }
        if (json.cod == 429) {
  ...
  parseOpenWeather: function(json) {   
    ...
    weather.condition.icon = this.weatherIconSafely(json.weather[0].icon, this.resolveOpenWeatherIcon); //34
  ...
  getOpenWeatherQueryString: function(url) {
    ...
    if (this.isCoordinate(loc)) {
      ...
      loc = loc.split(',');
      return url + "q=" + loc[0].trim() + "," + loc[1].trim() + "&APPID=" + APIKey;
    }
